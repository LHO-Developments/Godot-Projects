...
func check_game_state() -> void:
	...
	game_ui.game_over(_move_made, best);
...
func place_player_on_tile(title_coord: Vector2i) -> void:
	player.position = Vector2(title_coord * _tile_size);
	_player_tile = title_coord;

func get_atlas_cord(lt: TileLayers.LayerType) -> Vector2i:
	match lt:
...
func add_tile(lt: TileLayers.LayerType, tc: Vector2i, map: TileMapLayer) -> void:
	var atlas_coord: Vector2i = get_atlas_cord(lt);
